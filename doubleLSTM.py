...
regressorBi.add(LSTM(units=50,return_sequences=True))
regressorBi.add(Dropout(0.2))
regressorBi.add(LSTM(units=50))
regressorBi.add(Dropout(0.2))
regressorBi.add(Dense(units=1))
regressorBi.compile(optimizer='rmsprop',loss='mean_squared_error')
regressorBi.fit(X_train,y_train,epochs=50,batch_size=32)
...
predicted_stock_price = regressorBi.predict(X_test)
predicted_stock_price = sc.inverse_transform(predicted_stock_price)
plt.plot(test_set,color='red',label = 'Real pufa Price')
plt.plot(predicted_stock_price,color='blue',label = 'Predocted pufa Stock price')
plt.title('IBM Stock Price Prediction(LSTM')
plt.xlabel('Time')
plt.ylabel('pufa Stock Price')
plt.legend()
plt.show()
